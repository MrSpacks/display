...
# Параметры подсветки дисплея (если у модуля есть BL/LED пин).
# При enabled=False поведение не меняется.
BACKLIGHT = {
    'enabled': False,
    'gpio': 18,
    'pwm_hz': 1000,
...
# Целевая частота кадров для декодирования видео.
# Для ST7789 320x240 обычно практичный диапазон 24-30 FPS.
VIDEO_TARGET_FPS = 30

# Количество потоков ffmpeg для видеодекодирования: 0 = auto.
VIDEO_DECODER_THREADS = 0

# Пауза главного цикла (секунды).
MAIN_LOOP_SLEEP_VIDEO = 0.0015
MAIN_LOOP_SLEEP_IDLE = 0.05
